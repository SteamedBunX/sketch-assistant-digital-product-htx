import { AssistantPackage, RuleContext, RuleDefinition } from '@sketch-hq/sketch-assistant-types'
  ...
  rule: async (context) => {
    ...
    for (const page of utils.objects.page) {
      for (const artboard of page.layers) {
        if (artboard._class !== 'artboard') { continue; };
        if (!artboard.name.startsWith(page.name + " - "))
          context.utils.report(`Artboard "${artboard.name}"'s Name should start with "${page.name} - "`);
      }
    }
  },
  ...
  rule: async (context) => {
    ...
    for (const page of utils.objects.page) {
      ...
      if (!paragraph.match(regex)) {
        context.utils.report(`Page "${page.name}" should start with an emoji`);
      }
    }
  },
...
const groupNameCheck: RuleDefinition = {
  rule: async (context) => {
    ...
    for (const group of utils.objects.group) {
      if (group.name === "Group") {
        reportGroupNameViolation(context, group.name);
        continue;
      }
      ...
      if (nameComponents[nameComponents.length - 1] === "Copy") {
        reportGroupNameViolation(context, group.name);
      }
    }
  },
...
function reportGroupNameViolation(context: RuleContext, groupName: String) {
  context.utils.report(`Group name for "${groupName}" shouldn't be default`);
}
      ...
      if (page.name === 'Symbols') {
        for (const symbol of page.layers) {
          if (symbol.name.split('/').length <= 1) {
            context.utils.report(`Symbol "${symbol.name}" should use forward slash grouping`);
          }
        }
        return;
      }
