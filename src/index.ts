    ...
    for (const page of utils.objects.page) {
      ...
      for (const artboard of page.layers) {
        if (artboard._class !== 'artboard'
          ...
          || isArchived(artboard.name)) { continue; };
  ...
  rule: async (context) => {
    ...
    for (const page of utils.objects.page) {
      if (page.name === "Symbols"
        || isArchived(page.name)) { continue; };

      const firstCharacter = [...page.name][0];
      const paragraph = "" + firstCharacter;
      if (!paragraph.match(regex)) {
        context.utils.report(`Page "${page.name}" should start with an emoji`, page);
      }
    }
  },
...
const groupNameCheck: RuleDefinition = {
  rule: async (context) => {
    ...
    for (const group of utils.objects.group) {
      if (isArchived(group.name)) { continue; };
      if (group.name === "Group") {
    ...
    for (const page of utils.objects.page) {
      if (page.name === 'Symbols') {
        for (const symbol of page.layers) {
          if (isArchived(symbol.name)) { continue; };
